...
function onMessage(event) {
    ...
    if (device.action === "toggle") {
        ...
        var statusText = node.children[2];
         if (device.status === "On") {
            statusText.innerHTML = ": " + device.status + " (<a href=\"#\" OnClick=toggleDevice(" + device.id + ")>Turn off</a>)";
...
function printDeviceElement(device) {
   ...
   if (device.status === "On") {
    ...
    removeDevice.innerHTML = "<a href=\"#\" OnClick=removeDevice(" + device.id + ")>Remove Entry</a>";
